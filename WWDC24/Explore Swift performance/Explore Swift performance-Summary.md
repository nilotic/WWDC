...
- 세션 주제: **Explore Swift performance**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2024/10217/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Introduction`: My name is John McCall and today we are going to explore Swift performance.
  - `Agenda`: So in this talk, we're going to explore the low-level performance of Swift.
  - `What is performance?`: So what do we mean by performance?
  - `Low-level principles`: With all that said, let’s dig on those four principles of low-level performance.
...
- 세션 기반 체크포인트
  ...
  - `Function calls`: The first is function calls.
  - `Memory allocation`: That ties nicely into the next low-level principle we want to look at, which is memory allocation.

## ✅ 핵심 정리

- 이 세션은 **Explore Swift performance**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
