...
- 세션 주제: **Explore App Store server APIs for In-App Purchase**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2024/10062/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Introduction`: Hello and welcome to "Explore App Store server APIs for In-App Purchase".
  - `Purchase lifecycle`: Let's get started with the purchase lifecycle.
  - `Delivering content`: First of all, let's understand what the content delivery workflow looks like with a server.
  - `Subscriptions and Offers`: To that end, in late 2023 we added a handful of new fields to the transaction data provided by our server APIs and on-device via StoreKit.
...
- 챕터 흐름 요약
  ...
  - **Introduction**
    - Hello and welcome to "Explore App Store server APIs for In-App Purchase".
    - The App Store server API allows you to make requests from your server to the App Store server.
  - **Purchase lifecycle**
    - Let's get started with the purchase lifecycle.
    - There are a variety of in-app purchase types; here is a brief overview.
  - **Delivering content**
    - First of all, let's understand what the content delivery workflow looks like with a server.
    - Enable App Store Server Notifications V2 for your app.
  - **Subscriptions and Offers**
    - Next, I'll discuss subscriptions and offers, including how you can use offers to attract and retain customers to your subscription products.
    - To that end, in late 2023 we added a handful of new fields to the transaction data provided by our server APIs and on-device via StoreKit.
...
- 이 세션은 **Explore App Store server APIs for In-App Purchase**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
