...
- 세션 주제: **Create custom visual effects with SwiftUI**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2024/10151/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Introduction`: Hello, and welcome to Create Custom Visual Effects in SwiftUI.
  - `Scroll effects`: We're going to start with something we're all probably familiar with, scrolling.
  - `Color treatments`: Next, let’s talk about how you can bring color effects to your app.
  - `View transitions`: Next let’s talk about creating custom transitions.
...
- 세션 기반 체크포인트
  ...
  - `Text transitions`: Thanks, Phillip.
  - `Metal shaders`: And there's another powerful graphics API in SwiftUI, that offers even more fine-grained control, Shaders.

## ✅ 핵심 정리

- 이 세션은 **Create custom visual effects with SwiftUI**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
