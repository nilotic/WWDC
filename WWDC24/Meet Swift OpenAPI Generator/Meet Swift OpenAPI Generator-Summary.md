...
- 세션 주제: **Meet Swift OpenAPI Generator**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2023/10171/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Considerations when making API calls`: But in order to make the right network request, there's a lot to consider.
  - `Meet OpenAPI`: Using a more formal and structured description of APIs can help eliminate ambiguity.
  - `Making API calls from your app`: Let's take a look at how we can use Swift OpenAPI Generator in a simple iOS app.
  - `Adapting as the API evolves`: As new features are added to the server, its API will evolve.
...
- 챕터 흐름 요약
  ...
  - **Considerations when making API calls**
...
- 세션 기반 체크포인트
  ...
  - `Testing your app with mocks`: The generated protocol is named APIProtocol, so we'll start by defining a new MockClient type that adopts this protocol.
  - `Server development in Swift`: The server is a simple Swift package, which uses the Swift OpenAPI Generator package plug-in for code generation.

## ✅ 핵심 정리

- 이 세션은 **Meet Swift OpenAPI Generator**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
