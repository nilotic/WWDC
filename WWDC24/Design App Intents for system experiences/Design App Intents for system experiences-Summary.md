...
- 세션 주제: **Design App Intents for system experiences**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2024/10176/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Intro`: Hi, my name is Ray and I'm a designer on the Apple design team.
  - `Which App Intents to make`: Let's start with which app intents to make.
  - `Structuring App Intents`: starting with parameter types.
...
- 챕터 흐름 요약
  ...
  - **Which App Intents to make**
    - Let's start with which app intents to make.
    - Previously, app intents were meant to be the most habitual tasks in your app that could be useful outside of your app.
  - **Structuring App Intents**
    - starting with parameter types.
    - As we saw earlier, an app intent contains a parameter summary, which contains the inputs that your intent uses to run the task.
...
- 이 세션은 **Design App Intents for system experiences**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
