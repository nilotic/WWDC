...
- 세션 주제: **Animate with springs**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2023/10158/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Why springs`: Let's examine a context where we have a few animations: a simple toggle.
  - `How springs work`: When we use a spring animation, we're modeling it with the motion of an object attached to a spring.
  - `How to use springs`: Because springs are such a great tool for animations, we now use them as the default animation in SwiftUI, so all you need to do is call withAnimation to start with a spring.
...
- 이 세션은 **Animate with springs**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
