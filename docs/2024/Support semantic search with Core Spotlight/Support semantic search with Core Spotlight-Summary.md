...
- 세션 주제: **Support semantic search with Core Spotlight**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2024/10131/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Introduction`: Hi, I’m Jennifer, an engineer working on the Spotlight team.
  - `Searchable content`: The first step to building a great search experience is to donate searchable content to Spotlight that represents what people are going to want to search in your app.
  - `Demo: Creating an index delegate extension`: Starting in the journaling app, which has already been configured to donate searchable content with our bundle ID: The first thing to do, is create a new target: Navigate to the...
  - `Results and suggestions`: Now that you’ve indexed your searchable items, it’s time to support the search experience in your user interface.
...
- 챕터 흐름 요약
  ...
  - **Demo: Creating an index delegate extension**
...
- 세션 기반 체크포인트
  ...
  - `Ranking`: Now that search is working, you’ll want to think about improving the search experience over time, by donating signals that can improve the ranking of content that the user cares...
  - `Wrap-up`: We saw how to donate content to an entirely private, on-device search index.

## ✅ 핵심 정리

- 이 세션은 **Support semantic search with Core Spotlight**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
