...
- 세션 주제: **Tailor macOS windows with SwiftUI**
- 세션 링크: `https://developer.apple.com/videos/play/wwdc2024/10148/`
...
- 이 세션에서 특히 봐야 할 포인트
  ...
  - `Anatomy of a window`: A typical window has a few recognizable components.
  - `App window structure`: Now, I will use new SwiftUI APIs to further tune the windows for my app's content.
  - `Style window toolbars`: It is created from a WindowGroup with a content view.
  - `Refine window behaviors`: Next, I will refine the behaviors of the About window.
...
- 챕터 흐름 요약
  ...
  - **Anatomy of a window**
    ...
    - At the top of the window, there is a toolbar to host the window controls, the title, and toolbar items.
  ...
  - **Style window toolbars**
    ...
    - Right now, the window shows the toolbar and title.
  - **Refine window behaviors**
    - Next, I will refine the behaviors of the About window.
    - Every Mac app has an About window, accessed from the main menu.
  - **Adjust window placement**
    - Last, I will adjust exactly where they appear on screen.
    - This is especially important for the video player window.
...
- 세션 기반 체크포인트
  ...
  - `Adjust window placement`: Last, I will adjust exactly where they appear on screen.

## ✅ 핵심 정리

- 이 세션은 **Tailor macOS windows with SwiftUI**를 단순 기능 소개가 아니라, 실제 제품/서비스 코드에 적용할 때 필요한 설계 판단 관점까지 연결해 이해하는 데 가치가 있습니다.
